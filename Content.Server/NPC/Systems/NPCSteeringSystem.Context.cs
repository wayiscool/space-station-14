...
public sealed partial class NPCSteeringSystem
{
    ...
    private bool TrySeek(
        ...
        float frameTime,
        ref bool forceSteer,
        ref float moveMultiplier) // Starlight
    {
        ...
        var targetCoordinates = steering.DirectMove ? steering.Coordinates : GetTargetCoordinates(steering); // Starlight Edit
        ...
        if (!targetCoordinates.IsValid(EntityManager))
        {
            if (!directMove && steering.CurrentPath.TryPeek(out var poly) && // Starlight Edit
        ...
        if (arrived)
        {
            ...
            if (!directMove && steering.CurrentPath.TryPeek(out var node) && !IsFreeSpace(uid, steering, node)) // Starlight Edit
            {
            ...
            if (!directMove && steering.CurrentPath.Count > 0) // Starlight Edit
            {
                forceSteer = true;
                steering.CurrentPath.Dequeue();
